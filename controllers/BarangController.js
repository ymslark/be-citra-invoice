...
class BarangController {
  ...
  async getAll(req,res){
    try {
      const Barangs = await Barang.find({isActive:true}).sort({kode: 1}).select('nama kode')
      if(!Barangs) {throw {code: 404, message: 'BARANG_NOT_FOUND'}}

      // const namaList = Barangs.map(barang => barang.nama)
      // const kodeList = Barangs.map(barang => barang.kode)
  ...
  async getAllNama(req,res){
    try {
      const Barangs = await Barang.find({isActive:true}).sort({kode: 1}).distinct('nama')
      if(!Barangs) {throw {code: 404, message: 'BARANG_NOT_FOUND'}}
      return res.status(200)
              .json({
                  status: true,
  ...
  async getAllCode(req,res){
    try {
      const Barangs = await Barang.find({isActive:true}).sort({kode: 1}).distinct('kode')
      if(!Barangs) {throw {code: 404, message: 'BARANG_NOT_FOUND'}}
      return res.status(200)
              .json({
                  status: true,
  ...
  async bulkStore(req,res){
    try {
      ...
      // Query database sekali untuk semua kode
      const existingKodeBarangs = await Barang.find({ kode: { $in: kodeList } });
      const existingNamaBarangs = await Barang.find({ nama: { $in: namaList } });
      // if(existingBarangs.length > 0) throw { code: 500, message: 'KODE_BARANG_ALREADY_EXIST' }
      console.log(existingBarangs)
      // Buat set dari kode yang sudah ada
      const existingKodeSet = new Set(existingKodeBarangs.map(barang => barang.kode))
      const existingNamaSet = new Set(existingNamaBarangs.map(barang => barang.nama))

      if (existingKodeSet.size > 0) {
        throw { code: 500, message: 'KODE_BARANG_ALREADY_EXIST', existingKode: [...existingKodeSet] }
      }
      if (existingNamaSet.size > 0) {
        throw { code: 500, message: 'NAMA_BARANG_ALREADY_EXIST', existingNama: [...existingNamaSet] }
      }
      ...
      return res.status(error.code||500)
                  .json({
                      ...
                      message: error.message,
                      existingKode: error.existingKode || [],
                      existingNama: error.existingNama || [] })
    }
  }
  ...
  async filterData(req, res) {
    try {
      ...
      const query = buildFilterQuery(
        ...
        { searchFields: ['kode', 'nama'], defaultLast30Days, isActive }
      );
  
      const data = await Barang.paginate(query, {
        select : '-__v -isActive -createdAt -updatedAt',
        page,
        limit,
        sort: { tanggal: order },
      });
      if (!data) throw {code: 402, message:'FAILED_FETCH_DATA_BARANG'}
      data.barangs = data.docs
      delete data.docs
      res.status(200).json({ 
                          status: true,
                          message: 'SUCCESS_FETCH_DATA_BARANG', 
                          ...data })
    } catch (error) {
      console.log(error)
  ...
  async deleted(req,res){
    try {
      const limit = req.query.limit || 5
      const page  = req.query.page || 1
      const Barangs = await Barang.paginate({isActive:false},{
                                              sort: {kode: -1},
                                              limit: limit,
                                              page : page
                                            })
